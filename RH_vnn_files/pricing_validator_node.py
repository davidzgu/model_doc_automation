...
def pricing_validator_tool_node(
    ...
    sensitivity_state_fn = create_mcp_state_tool_wrapper(
        mcp_tool_name="run_sensitivity_test_to_file",
        server_script_path=server_path,
        input_arg_map={
            "greeks_results_path": "input_path",
            "output_dir": "output_dir",
        },
        output_key="sensitivity_results_path",
    )

    stress_state_fn = create_mcp_state_tool_wrapper(
        mcp_tool_name="run_stress_test_to_file",
        server_script_path=server_path,
        input_arg_map={
            "greeks_results_path": "input_path",
            "output_dir": "output_dir",
        },
        output_key="stress_results_path",
    )

    pnl_state_fn = create_mcp_state_tool_wrapper(
        mcp_tool_name="run_pnl_test_to_file",
        server_script_path=server_path,
        input_arg_map={
            "greeks_results_path": "input_path",
            "output_dir": "output_dir",
        },
        output_key="pnl_results_path",
    )
    ...
    sensitivity_update = sensitivity_state_fn(state=state)
    state.update(sensitivity_update)

    stress_update = stress_state_fn(state=state)
    state.update(stress_update)

    pnl_update = pnl_state_fn(state=state)
    state.update(pnl_update)
